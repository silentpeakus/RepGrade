...
| GPA contribution | Numeric, feeds into rolling exercise GPA and overall GPA | Needs a defined formula (recency weighting? simple average? — decision needed) |
...
**Decision: ~15–20 exercises for v1**, spanning major compounds and their most common accessories. Suggested starting list (to be confirmed against your actual target user/audience before Phase 0 begins):
...
This is a placeholder list, not a final one — finalize it before Phase 0 rubric work begins, since rubric-writing effort scales directly with this list.
...
- Rolling GPA = function over `SetEntry.Grade` for a given `Exercise`, over a given `Athlete`, over a configurable time window
...
- Confirm final v1 exercise list (~15–20 lifts — draft in Section 4.2.1).
- Define the per-exercise rubric (joints tracked, fault thresholds, grade bands) for each.
...
- Define GPA formula (Section 7.2 — still open).
...
| Question | Decision |
|---|---|
| Exercise list scope | ~15–20 exercises (compounds + common accessories). Draft list in Section 4.2.1 — needs final confirmation against target user before Phase 0. |
...
### 7.2 Still Open — Needed Before Roadmap Can Be Time-Estimated

1. **GPA formula:** Simple rolling average, recency-weighted, or best-of-recent-N? This is a product decision as much as a technical one — it determines what behavior you're incentivizing (e.g., recency-weighting rewards current form over historical average, which probably matches the product's intent better than a flat average, but worth deciding deliberately). Note: this is the *overall* exercise/athlete GPA formula — it's separate from the Stability Gate's own 2-session window (Section 4.2.2), which is fixed regardless of how the headline GPA is computed.
2. **Final v1 exercise list confirmation:** The draft list in Section 4.2.1 is a placeholder — confirm or revise before Phase 0 rubric-writing begins, since that work scales directly with list length.
3. **Max video length per set:** Suggested 60–90 seconds (Section 4.1) — confirm this covers realistic set lengths for your target rep ranges without bloating upload/processing time.
...
*End of draft. Once Section 7.2 (Remaining Open Questions) is resolved, this document should be updated and the roadmap in Section 6 re-sequenced/estimated with actual time/resource figures.*
