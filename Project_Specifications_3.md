...
| Cues | 2–4 short text strings, each tied to a specific rep/timestamp where possible | **Decision: cues are a mix of positive and corrective, not corrective-only.** See Section 4.2.3 for the rule. This is the qualitative LLM layer. |
...
**4.2.3 Cue Composition — Positive and Corrective (decided for v1)**

The original cue design only ever surfaced what went wrong. **Decision: cues must reflect what the rubric actually found, including faults that passed cleanly — not just faults that failed.** This isn't a tone preference; it's a data completeness fix. Every rubric (Section 4.2.1, and the per-exercise rubric documents) already computes a pass/fail result per fault per rep — that data exists regardless of this decision. The gap was in what the LLM coaching layer was instructed to do with it: it was only ever prompted to explain failures, discarding the passing results entirely.

**Composition rule:**
- If at least one tracked fault passed cleanly across the entire set (zero flagged reps for that fault), the cue list includes at least one positive cue naming that specific fault by name — not generic praise. Compare: "Your depth was consistent across all 8 reps" (usable) vs. "Great set!" (not usable — doesn't tell the athlete what to keep doing).
- **Ordering: positive cues lead, corrective cues follow** — e.g., "Your depth was solid across the whole set. Now work on the back rounding that showed up on reps 4 and 6." This ordering is a deliberate choice (matches how people actually receive correction better) and should not be treated as an LLM-prompt detail to be silently reordered later — it's a product decision, same tier as the other 4.2.2 decisions.
- If every tracked fault failed at least once, the cue list may be corrective-only for that set — do not manufacture a positive cue where the rubric didn't find one. A B+ set with two minor faults can lead with whichever fault passed most cleanly, even if it's not a perfect 0-fault result; the bar for "positive cue" is "this fault didn't appear," not "the whole set was flawless."
- Total cue count stays at 2–4 (per the existing output spec) — adding positive cues does not mean adding more total cues; it means changing the mix of the same 2–4.

**Why this isn't just a wording change:** Section 5's LLM Coaching Layer is described as explaining "what the pose layer already detected" — that description was implicitly fault-only. This decision clarifies that "what was detected" includes clean passes, and the LLM prompt design in Phase 1 needs to be built around the full pass/fail result set from day one, not patched on afterward. Retrofitting this later would mean reworking the coaching-layer prompt rather than just adding a feature.
...
LLM Coaching Layer (Claude or similar)
  - Receives: rubric scores + rep-level fault flags (pass AND fail results — see Section 4.2.3) + (optionally) key frames
  - Produces: human-readable cues mixing what passed cleanly with what needs correction (positive cues lead, per Section 4.2.3), the "why," and tone-appropriate coaching language
  - Does NOT independently re-score form from raw video in v1 — it explains/contextualizes what the pose layer already detected, including clean passes, not just faults. (This keeps grading reproducible and auditable; an LLM re-grading the same video twice can drift, which is bad for a GPA product where consistency *is* the trust mechanic.)
     │
     ▼
...
- **Validation order: squat, bench press, deadlift, overhead press first.** These 4 were chosen because each stresses a different pose-tracking capability the pipeline needs across the full 13-exercise set: squat (multi-joint angle tracking, per-athlete style variance), bench press (bar-path as the primary signal), deadlift (relative-rate timing between two keypoints, not just position thresholds), overhead press (front-view capture, bilateral L/R symmetry). If these 4 validate, the remaining 9 reuse the same underlying patterns rather than introducing new ones.
- Define the per-exercise rubric (joints tracked, fault thresholds, grade bands) for each of the 13 confirmed v1 exercises (Section 4.2.1) — but calibrate and validate squat/bench/deadlift/OHP first before sinking further tuning time into the remaining 9.
...
- Build the LLM coaching-cue layer (prompt design, rep-fault-to-cue mapping) — prompt must surface both passed and failed rubric checks per Section 4.2.3, not faults only.
...
| Cue composition | **Positive + corrective mix, positive cues lead.** Cues reflect both passed and failed rubric checks, not faults only. See Section 4.2.3. |
